...
DISCORD_BOT_TOKEN     = os.environ.get("DISCORD_BOT_TOKEN",     "MTQ5OTg4Nzg3NDU4NDIxOTc3OQ.G08qG4.eHK6Pqo5gJ16EogHWdshQP34QT9SpOG5nw-vLU")
...
def check_for_changes():
    ...
    if state is None:
        print(f"  First run — {len(tracks)} songs on Spotify, {len(INITIAL_KNOWN_IDS)} known. Initializing...")
        state = {}
        for track_id, t in tracks.items():
            state[track_id] = {"thread_id": None, "name": t["name"], "artist": t["artist"]}
        save_state(state)
